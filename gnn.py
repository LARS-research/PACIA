...
class HyperSetEncoder(nn.Module):
    ...
    def cat_label(self,all_emb,labels):
        class_features_0 = torch.index_select(all_emb, 0, self._extract_class_indices(labels,0))
        class_features_1 = torch.index_select(all_emb, 0, self._extract_class_indices(labels,1))
        all_emb=torch.cat((class_features_0,class_features_1),0)
        k0=int(class_features_0.size(0))
        n,d=all_emb.size()
        l0=torch.zeros((k0,1),device=all_emb.device)
        l1=torch.ones((k0,1),device=all_emb.device)
        
        ln=torch.cat((l1,l0),-1)
        del l0,l1
        l0=torch.zeros((n-k0,1),device=all_emb.device)
        l1=torch.ones((n-k0,1),device=all_emb.device)
...
class GNNM(nn.Module):
    def __init__(
        ...
        self.readout_config=GraphReadoutConfig(readout_type=config.readout_config.readout_type,use_all_states=False, output_dim = 512)

        if self.readout_config.use_all_states:
        ...
        self.layer_list = OrderedDict()
        for l in range(config.gnn_config.num_layers):
            modulator_w=HyperSetEncoder(inp_dim=self.readout_config.output_dim+2,hidden_dim=self.readout_config.output_dim,out_dim=readout_node_dim,dropout=hdp)
            modulator_b=HyperSetEncoder(inp_dim=self.readout_config.output_dim+2,hidden_dim=self.readout_config.output_dim,out_dim=readout_node_dim,dropout=hdp)
            gnnl=GNNBlock(config.gnn_config)
            self.add_module('gnn{}'.format(l), gnnl)
            self.add_module('readout{}'.format(l), make_readout_model(self.readout_config,readout_node_dim,))
            self.add_module('modw{}'.format(l), modulator_w)
            self.add_module('modb{}'.format(l), modulator_b)



    def forward(self, node_features, adj_lists,batchin,sup=True,wbl=[],sl=None) -> List[torch.Tensor]:
        """
        args:
            node_representations: float tensor of shape (num_nodes, config.hidden_dim)
            adj_lists: List of (num_edges, 2) tensors (one per edge-type)
        ...
        """
        # We may need to introduce additional edges to make everything bidirectional:
        if self.gnn_config.make_edges_bidirectional:
            adj_lists = [
                torch.cat((adj_list, torch.flip(adj_list, dims=(1,))), dim=0)
                for adj_list in adj_lists
            ]
        ...
        # Actually do message passing:
        assert sup==(wbl==[]) and (sup == (sl is not None))
        cur_node_representations = node_features
        all_node_representations: List[torch.Tensor] = [cur_node_representations]
        wb_list=[]
        for l in range(self.gnn_config.num_layers):
            cur_node_representations=self._modules['gnn{}'.format(l)](
                node_representations=cur_node_representations,
                adj_lists=adj_lists,
            )
            inp=self._modules['readout{}'.format(l)](node_embeddings=cur_node_representations,node_to_graph_id=batchin.node_to_graph,num_graphs=batchin.num_graphs)
            if sup:
                inp,k0=self._modules['modw{}'.format(l)].cat_label(inp,sl)
                w=self._modules['modw{}'.format(l)](inp,k0).unsqueeze(0)
                b=self._modules['modb{}'.format(l)](inp,k0).unsqueeze(0)
                wb_list.append(w)
                wb_list.append(b)
            else:
                w=wbl[l*2]
                b=wbl[l*2+1]
            cur_node_representations=cur_node_representations*(1+w)+b
            
            all_node_representations.append(cur_node_representations)


        return all_node_representations,wb_list

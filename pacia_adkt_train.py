...
def make_trainer_config(args: argparse.Namespace) -> PACIA_ADKTModelTrainerConfig:
    return PACIA_ADKTModelTrainerConfig(
...
def main():
    ...
    out_dir, dataset, aml_run = set_up_train_run(
        f"PACIA_ADKTModel_{config.used_features}", args, torch=True
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_trainer = PACIA_ADKTModelTrainer(config=config).to(device)

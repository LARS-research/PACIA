...
from fs_mol.modules.gnn import GNN,GNNM, GNNConfig, add_gnn_model_arguments, make_gnn_config_from_args
...
class GraphFeatureExtractor(nn.Module):
    def __init__(self, config: GraphFeatureExtractorConfig,hdp=0):
        super().__init__()
        self.config = config
    ...
    def forward(self, input: FSMolBatch,sup=True,wbl=[],sl=None) -> torch.Tensor:
...
class GraphFeatureExtractorM(nn.Module):
    def __init__(self, config: GraphFeatureExtractorConfig,hdp=0):
        super().__init__()
        self.config = config
        ...
        self.gnn = GNNM(self.config,hdp=hdp)
    ...
    def forward(self, input: FSMolBatch,sup=True,wbl=[],sl=None) -> torch.Tensor:
        ...
        # ----- Message passing layers:
        all_node_representations,wbl1 = self.gnn(initial_node_features, input.adjacency_lists,sup=sup,batchin=input,wbl=wbl,sl=sl)
        ...
        if self.final_norm_layer is not None:
            mol_representations = self.final_norm_layer(mol_representations)
        return mol_representations,wbl1